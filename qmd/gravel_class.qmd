...
```{r gravel-style-functions, echo=FALSE}
...
```{r gravel-style-tree, fig.height=10, fig.width=10}
...
style_table <- dcast(setDT(style_class), rowid(restyle) ~ restyle, value.var = "model")[, .SD, .SDcols = cluster_labels] %>%
  clean_names()
style_table[, trail := trail[order(trail)]]
style_table[, endurance := endurance[order(endurance)]]
style_table[, all_road := all_road[order(all_road)]]
setnames(style_table, new = c("Trail", "Endurance", "All-Road"))

style_table %>%
  kable(caption = "Classification of bikes based on frame sizes that fit me (generally size M or 54-56 cm)") %>%
