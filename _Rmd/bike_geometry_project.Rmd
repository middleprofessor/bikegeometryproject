...
compute_head_tube_length <- function(bike){
  ...
  cr <- ifelse(bike[1, material] == "steel" | bike[1, material] == "titanium", 14, 0)
  cr <- bike[, lower_headset_stack]
  cr.v <- cr * sin(hta)
  beta <- acos(rake/ac)
  delta <- pi/2 - hta
  gamma <- beta - delta
...
estimate_axle_crown <- function(bike, material){
  ...
  headset_l <- bike[, lower_headset_stack]
...
clean_frame_size <- function(bike){
  ...
  bike[, frame_size := str_remove_all(frame_size, " ")]
...
clean_and_compute <- function(bike){
  ...
  bike[!is.na(as.numeric(dropout)), dropout_numeric := as.numeric(dropout)]
  bike[, chainstay_mid := ifelse(is.na(dropout_numeric), chainstay_length, chainstay_length + dropout_numeric/2)]
  bike[, chainstay_table := 
         ifelse(is.na(dropout_numeric),
                chainstay_length,
                paste(chainstay_length, chainstay_length + dropout_numeric, sep = " - "))]
  ...
  # rear center is the horizontal component of chainstay
  bike[, rear_center := sqrt(chainstay_length^2 - bottom_bracket_drop^2)] # use chainstay_mid
...
d_scores <- function(Y,
  ...
  loading_names <- rownames(loadings)
  rc <- loading_names[4]
  fc <- loading_names[3]
  hta <- loading_names[5]
  style_levels <- c("racy", "relaxed", "xtra_rowdy", "rowdy")[1:k]
  relaxed <- which.max(loadings[rownames(loadings) == rc])
  relaxed <- class_dt[model == "Salsa Vaya 2022", init_cluster]
  colnames(scores_scaled)[relaxed] <- "relaxed"
  xtra_rowdy <- which.max(loadings[rownames(loadings) == fc])
  xtra_rowdy <- class_dt[model == "Specialized Chisel 2025", init_cluster]
  colnames(scores_scaled)[xtra_rowdy] <- "xtra_rowdy"
  racy <- which.max(loadings[rownames(loadings) == hta])
  racy <- class_dt[model == "Factor Aluto 2025", init_cluster]
...
```{r classify}
assign_class_names <- function(kmeans_class, k=4){
  
  style_levels <- c("racy", "relaxed", "rowdy", "xtra_rowdy")
  
  racy <- class_dt[model == "Time ADHX 2025", init_cluster]
  relaxed <- kmeans_class[model == "Tumbleweed Stargazer Gen 2 2025", init_cluster]
  xtra_rowdy <- kmeans_class[model == "Specialized Chisel 2025", init_cluster]
  rowdy <- setdiff(1:4, c(racy, relaxed, xtra_rowdy))

  kmeans_class[get(cluster_col) == racy, style := "racy"]
  kmeans_class[get(cluster_col) == relaxed, style := "relaxed"]
  kmeans_class[get(cluster_col) == rowdy, style := "rowdy"]
  kmeans_class[get(cluster_col) == xtra_rowdy, style := "xtra_rowdy"]
...
kmeans_classifier <- function(dt,
  ...
  # name clusters
#  kmeans_class <- assign_class_names(kmeans_class)
  ...
  #PCA scores
  E <- eigen(cor(Y))$vectors
  pc_scores <- Y %*% E[, 1:2]
  kmeans_scores[, pc1 := pc_scores[, 1]]
  kmeans_scores[, pc2 := pc_scores[, 2]]
...
```{r reclassify-relaxed, echo=FALSE}
reclassify_relaxed <- function(dt,
                               dt_class,
                               k=2,
                               method = "rkm",
                               classifier_cols = c(
  ...
  dt_sub <- dt[dt_class$kmeans_style == "relaxed"]
  if(sag == TRUE){
    classifier_cols <- paste(classifier_cols, "sag", sep = "_")
  }
  # dt should be my_fit
  ...
  model <- dt_info[, model]
  Y <- dt_sub[my_fit == TRUE, .SD, .SDcols = classifier_cols] |>
    scale()
  row.names(Y) <- model
  ...
  rowdy <- setdiff(1:2, relaxed)
  explore_table[rkm_cluster == relaxed, restyle := "relaxed"]
  explore_table[rkm_cluster == rowdy, restyle := "rowdy"]
  new_class <- merge(dt_class,
                     explore_table,
                     by = c("model", "frame_size"), all.x = TRUE)
#  new_class[, style := ifelse(style == "relaxed", restyle, style)]
  new_class[is.na(restyle), restyle := kmeans_style]
  new_class[, kmeans_style := restyle]
  return(new_class)
}
```
...
# Sag frames with suspension forks
...
```{r predicted-vs-spec-size, eval = FALSE}
deviation <- 4
...
```{r what-models-dont-have-std-sizes, eval=FALSE}
na_models <- geobike[ , .(all_na = all(is.na(h176))), by = model][all_na == TRUE, model]

```

```{r compare-short-mid-chainstay, eval=FALSE}
x <- merge(geobike_short[my_fit == TRUE, .SD, .SDcols = c("model", "kmeans_style")],
           geobike_mid[my_fit == TRUE, .SD, .SDcols = c("model", "kmeans_style")],
           by = "model")
```

```{r compare kmeans_relaxed vs mds2, eval=FALSE}
y_cols <- c("mds1", "mds2", "kmeans_rowdy", "kmeans_relaxed")
cor(geobike[my_fit == TRUE, .SD, .SDcols = y_cols])
```

# Import bikes

```{r import-bikes-excel-single}
import_bikes_excel_single <- function(style_folder = "gravel", exchange_rates){
  ...
  for(i in 1:length(bike_list)){
    ...
    bike_i[fork == "rigid" & axle_crown >= 475 & axle_crown < 490, fork_class := "100 mm"]
    bike_i[fork == "rigid" & axle_crown >= 494 & axle_crown < 512, fork_class := "120 mm"]
...
# mtbikes
    ...
    fork_classes <- c("0 mm", "20 mm", "30 mm", "40 mm", "50 mm", "60 mm", "80-120 mm", "100 mm", "110 mm", "100-110 mm", "100-120 mm", "120 mm", "100-130 mm", "120-130 mm", "130 mm", "100-140 mm")
...
reclassifier_cols <-  c("stack", "rear_center")
...
if(import_it != TRUE){
  ...
  reclassify <- FALSE
  if(reclassify == TRUE){k <- 3}else{k <- 4}
  sag_it <- TRUE
  ...
  if(reclassify == TRUE){
    kmeans_classes <- reclassify_relaxed(
      geobike[my_fit == TRUE],
      kmeans_classes,
      k = 2,
      method = "rkm", # km or rkm where rkm is robust
      classifier_cols = reclassifier_cols,
      sag = sag_it
    )
    k <- 4
  }
