...
my_fit[is.na(rider_min), .SD, .SDcols = c("model", "rider_min")]
```
...
#  geobike <- plyr::join(geobike, geobike_classes, by = "model")
  ...
  # average M is 175
  # average L is 183.5
  geobike[frame_size_orig == "M", .(M_low = median(rider_min, na.rm=TRUE))]
  geobike[frame_size_orig == "M", .(M_hi = median(rider_max, na.rm=TRUE))]
  geobike[frame_size_orig == "L", .(M_low = median(rider_min, na.rm=TRUE))]
  geobike[frame_size_orig == "L", .(M_hi = median(rider_max, na.rm=TRUE))]
  size_list_10 <- seq(150, 195, 5)
  # four sizes
  # 5.2 5.6, 5.10, 6.2
  # xs, s, m, l
  size_list_5 <- seq(160, 192, 8)
  size_list_3 <- seq(152, 200, 3)
  for(i in 1:nrow(geobike)){
    rider_min <- geobike[i, rider_min]
    rider_max <- geobike[i, rider_max]
    if(!is.na(rider_min)){
      for(j in 1:length(size_list_5)){
        if(size_list_5[j] >= rider_min & size_list_5[j] < rider_max){
          geobike[i, size_5 := size_list_5[j]]
        }
      }
      for(j in 1:length(size_list_3)){
        if(size_list_3[j] >= rider_min & size_list_3[j] < rider_max){
          geobike[i, size_3 := size_list_3[j]]
        }
      }
