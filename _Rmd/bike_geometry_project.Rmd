...
import_bikes_excel_single <- function(style_folder = "gravel", exchange_rates){
  ...
  for(i in 1:length(bike_list)){
    ...
    brand_i <- model_data[1, 2]
    model_i <- model_data[2, 2]
    year_i <- model_data[3, 2]
...
```{r generate_pages_site, eval=FALSE}
# ---- Config ---------------------------------------------------------------
site_base_url   <- "https://thegravelbikegeometryproject.com"   # your public URL
template_file   <- "qmd/bike_template.qmd"                           # model page template
data_rds_path   <- "rds/geobike.rds"                            # fallback if geobike not in env

site_output_dir <- "docs"                                        # Quarto output dir
bikes_dir       <- file.path(site_output_dir, "bikes")           # target for model pages

# Skip writing these so you don't clobber Quarto's files
write_sitemap   <- FALSE
write_robots    <- FALSE

# ---- Packages -------------------------------------------------------------
if (!requireNamespace("data.table", quietly = TRUE)) stop("Install 'data.table'")
if (!requireNamespace("fs", quietly = TRUE))         stop("Install 'fs'")
if (!requireNamespace("quarto", quietly = TRUE))     stop("Install 'quarto'")
...
# ---- Data source ----------------------------------------------------------
if (!exists("geobike")) {
  if (!file.exists(data_rds_path)) stop("geobike not in-memory and RDS not found at: ", data_rds_path)
  geobike <- readRDS(data_rds_path)
}
...
# ---- Dirs -----------------------------------------------------------------
dir_create(bikes_dir)

# ---- Unique pages + slugs -------------------------------------------------
# pages <- unique(geobike[, .(brand, model, year)])
pages <- unique(geobike[, model])
pages[, slug := slugify(model]
if (any(duplicated(pages$slug))) {
  pages[, dup_n := ave(seq_len(.N), slug, FUN = seq_along)]
  pages[!is.na(dup_n), slug := paste0(slug, "-", dup_n)]
}

# ---- Render each model page (inherits navbar/theme from project) ----------
# IMPORTANT: run this from the project root so Quarto sees your _quarto.yml
for (i in seq_len(nrow(pages))) {
  ...
  quarto::quarto_render(
    ...
    execute_params = list(
      ...
      year          = row$year,
      slug          = row$slug,
      data_path     = data_rds_path,
      site_base_url = site_base_url
    ),
    output_file = file.path(bikes_dir, paste0(row$slug, ".html"))
  )
}

# ---- Make bikes/index.qmd and render so it has your navbar ----------------
# We avoid tricky 'asis' chunks; this is a small QMD that reads a TSV we write now.
bix <- pages[, .(
  brand, model, year,
  link = sprintf('<a href="/bikes/%s.html">%s %s %s</a>', slug, brand, model, year)
)]
fwrite(bix, file.path(site_output_dir, "bikes_index.tsv"), sep = "\t")
...
knitr::kable(dt[, .(brand, model, year, link)],
format = "html",
table.attr = "class=\"table table-sm table-striped\"",
col.names = c("Brand","Model","Year","Page"))
'

writeLines(index_qmd, file.path("bikes", "index.qmd")) # in project space
quarto::quarto_render(file.path("bikes", "index.qmd"),
output_file = file.path(bikes_dir, "index.html"))

# Only run if you actually want a bikes-only sitemap file
if (isTRUE(write_sitemap)) {

  # Root <urlset>
  doc <- xml_new_root("urlset", xmlns = "http://www.sitemaps.org/schemas/sitemap/0.9")

  # Home
  url_home <- xml_add_child(doc, "url")
  xml_add_child(url_home, "loc", paste0(site_base_url, "/"))

  # Bikes index
  url_index <- xml_add_child(doc, "url")
  xml_add_child(url_index, "loc", paste0(site_base_url, "/bikes/index.html"))

  # One <url> per model page
  apply(pages[, .(slug)], 1, function(r) {
    u <- xml_add_child(doc, "url")
    xml_add_child(u, "loc", paste0(site_base_url, "/bikes/", r[["slug"]], ".html"))
  })

  out_path <- file.path(site_output_dir, "sitemap-bikes.xml")  # different name → no overwrite
  write_xml(doc, out_path, options = "format")
  message("Wrote bikes-only sitemap: ", out_path)
}

if (isTRUE(write_robots)) {
  robots <- c(
    "User-agent: *",
    "Allow: /",
    sprintf("Sitemap: %s/sitemap.xml", site_base_url),
    sprintf("Sitemap: %s/sitemap-bikes.xml", site_base_url)
  )
  writeLines(robots, file.path(site_output_dir, "robots-extra.txt"))
  # NOTE: we do NOT overwrite robots.txt; you can merge robots-extra.txt manually.
}
message("Done: model pages in docs/bikes, index with navbar, no sitemap/robots overwritten.")
```
