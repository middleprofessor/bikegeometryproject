...
read_geometry <- function(bike_label = "Alchemy Lycos 2023",
  ...
  keep_names <- c("model", "year", "frame_size",
                  ...
                  "rider_min", "rider_max", "rider_height"
  )
...
# [1] 159 167 175 183 190
  m <- median_sizes[3]
  geobike[, frame_size_std_m := ifelse(rider_min_pred <= m & rider_max_pred > m, TRUE, FALSE)]
  ...
  s <- median_sizes[2]
  geobike[, frame_size_std_s := ifelse(rider_min_pred <= s & rider_max_pred > s, TRUE, FALSE)]
  ...
  l <- median_sizes[4]
  geobike[, frame_size_std_l := ifelse(rider_min_pred <= l & rider_max_pred >= l, TRUE, FALSE)]
...
median_seat_tube <- c(
  xs = median(geobike[frame_size_letters == "XS", seat_tube_effective_length], na.rm=TRUE),
  s = median(geobike[frame_size_letters == "S", seat_tube_effective_length], na.rm=TRUE),
  m = median(geobike[frame_size_letters == "M", seat_tube_effective_length], na.rm=TRUE),
  l = median(geobike[frame_size_letters == "L", seat_tube_effective_length], na.rm=TRUE),
  xl = median(geobike[frame_size_letters == "XL", seat_tube_effective_length], na.rm=TRUE)
  
)
size_dt <- data.table(
  rider_size = median_sizes,
  eff_seat_tube = median_seat_tube
)
...
m1 <- lm(rider_size ~ eff_seat_tube, data = size_dt)
b <- coef(m1)
geobike[, rider_height := b[1] + b[2] * seat_tube_effective_length]

geobike[model == "Trek Checkmate 2025", .SD, .SDcols = c("frame_size", "rider_min", "rider_max", "rider_height")]
geobike[model == "Sklar SuperSomething 2025", .SD, .SDcols = c("frame_size", "rider_min", "rider_max", "rider_height")]
geobike[model == "Santa Cruz Stigmata CC 2025", .SD, .SDcols = c("frame_size", "rider_min", "rider_max", "rider_height")]

  # size midpoints
geobike[, rider_height := (rider_min + rider_max)/2]
m1 <- lm(rider_mid ~ seat_tube_effective_length +
           seat_tube_angle +
           head_tube_angle +
           stack_reach +
           chainstay_length,
         data = geobike, na.action=na.exclude)

ggplot(data = geobike,
       aes(x = seat_tube_effective_length,
           y = rider_mid,
           color = kmeans_style)) +
  geom_point()

b <- coef(m1)
geobike[, rider_height := b[1] +
          b[2] * seat_tube_effective_length +
          b[3] * seat_tube_angle +
          b[4] * head_tube_angle]
geobike[model == "Trek Checkmate 2025", .SD, .SDcols = c("frame_size", "seat_tube_effective_length", "rider_mid", "rider_height")]
geobike[model == "Sklar SuperSomething 2025", .SD, .SDcols = c("frame_size", "seat_tube_effective_length", "rider_mid", "rider_height")]
geobike[model == "Santa Cruz Stigmata CC 2025", .SD, .SDcols = c("frame_size", "seat_tube_effective_length", "rider_mid", "rider_height")]
geobike[model == "Fairlight Holt 2.0 2025", .SD, .SDcols = c("frame_size", "seat_tube_effective_length", "rider_mid", "rider_height")]

geobike[, m1_resid := residuals(m1)]
ggplot(data = geobike,
       aes(x = rider_mid,
           y = m1_resid)) + 
  geom_point()
...
for(i in 1:length(models)){
  ...
  p <- length(sizes)
  if(p > 1){
    rider_height <- geobike[model == models[i], rider_height]
    ...
    geobike[model == models[i], rider_min_est := mids[1:p]]
    geobike[model == models[i], rider_max_est := mids[2:(p+1)]]
  }
}

geobike[my_fit == TRUE, .SD, .SDcols = c("rider_min", "rider_min_est", "rider_max", "rider_max_est")]
geobike[rider_min_est <= 175 & rider_max_est > 175, std_size := "M"]
x <- geobike[my_fit == TRUE | std_size == "M", .SD, .SDcols = c("model", "frame_size", "my_fit", "std_size")]
...
frame_size_predictor <- function(geobike){
  ...
  x_cols <- c("stack", "reach", "head_tube_angle", "seat_tube_angle", "chainstay_length", "front_center", "head_tube_length", "top_tube_effective_length")
...
bike_i <- geobike[model == "Ventum GS1 2025", .SD, .SDcols = c("model", "frame_size", "rider_height", "rider_height_pred")]
