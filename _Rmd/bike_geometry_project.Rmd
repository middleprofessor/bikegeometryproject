...
import_bikes_excel_single <- function(style = "gravel",
  ...
  for(i in 1:nrow(bike_list)){
    ...
    bike_data <- read_excel(bike_path, range = "A1:B9", col_names = FALSE) |>
      data.table()
    bike_label_i <- paste(bike_data[1, 2], bike_data[2, 2], bike_data[3, 2])
    bike_range_i <- bike_data[9, 2] |> as.character()
    bike_i <- read_bike(bike_label = bike_label_i,
                        bike_range = bike_range_i,
...
import_bike_list <- function(style = "gravel",
  ...
  geobike[, frame_size_orig := frame_size]
  ...
  geobike[, frame_size := str_replace(frame_size, "3X", "XXX")]
  geobike[, frame_size := str_replace(frame_size, " - ", "/")]
  geobike[, frame_size := str_remove(frame_size, "-")]
  geobike[, frame_size := str_remove(frame_size, " ")]
  
  geobike[, frame_size_std_m := ifelse(rider_min < 175 & rider_max >= 175, TRUE, FALSE)]
  nrow(geobike[frame_size_std_m == TRUE])
  medium_list <- c("M","54","54(L)","54.7","54.5","M(55)","545","M/54", "53")
  geobike[is.na(frame_size_std_m), frame_size_std_m := ifelse(frame_size %in% medium_list, TRUE, FALSE)]

  geobike[, frame_size_std_l := ifelse(rider_min < 180 & rider_max >= 180, TRUE, FALSE)]
  nrow(geobike[frame_size_std_l == TRUE])
  large_list <- c("L", "560", "56", "L/57", "L/56", "57.5", "57", "L(57)")
  geobike[is.na(frame_size_std_l), frame_size_std_l := ifelse(frame_size %in% large_list, TRUE, FALSE)]

  # checks
  a <- data.table(geobike[frame_size_std_m == TRUE, ])
  b <- a[, .(N = .N), by = .(model)]
  b[N > 1, model]
  geobike[is.na(frame_size_std_l), .SD, .SDcol = c("model", "frame_size")]
  setdiff(geobike[frame_size_std_m == TRUE, model], unique(geobike$model))
  setdiff(unique(geobike$model), geobike[frame_size_std_l == TRUE, model])
  geobike[frame_size_std_l == TRUE, model]
  geobike[model == "Mason InSearchOf", .SD, .SDcol = c("frame_size", "frame_size_std_l", "rider_min", "rider_max")]
...
assign_std_size <- function(geobike){
  ...
  median_sizes <- c(
    # XS
    (median(geobike[frame_size == "XS", rider_min], na.rm=TRUE) +
      median(geobike[frame_size == "XS", rider_max], na.rm=TRUE))/2,
    # S
    (median(geobike[frame_size == "S", rider_min], na.rm=TRUE) +
      median(geobike[frame_size == "S", rider_max], na.rm=TRUE))/2,
    # M
    (median(geobike[frame_size == "M", rider_min], na.rm=TRUE) +
      median(geobike[frame_size == "M", rider_max], na.rm=TRUE))/2,
    # L
    (median(geobike[frame_size == "L", rider_min], na.rm=TRUE) +
      median(geobike[frame_size == "L", rider_max], na.rm=TRUE))/2,
    # XL
    (median(geobike[frame_size == "XL", rider_min], na.rm=TRUE) +
      median(geobike[frame_size == "XL", rider_max], na.rm=TRUE))/2
  ) |> round(0)

  # five sizes: xs, s, m, l, xl
  small_list <- c("S", "51", "52", "53")
  medium_list <- c("M","54","54(L)","54.7","54.5","M(55)","545","M/54", "55")
  large_list <- c("L", "560", "56", "L/57", "L/56", "57.5", "57", "L(57)")
...
#  geobike[, frame_size_std := as.numeric(NA)]
  ...
  # what are frame sizes for median size = small
# size_m_top_tube <- geobike[frame_size_std == median_sizes[3], top_tube_effective_length]
  
      # if(geobike[i, frame_size] %in% small_list){
      #   geobike[i, frame_size_std := median_sizes[2]]
      # }
      # if(geobike[i, frame_size] %in% medium_list){
      #   geobike[i, frame_size_std := median_sizes[3]]
      # }
      # if(geobike[i, frame_size] %in% large_list){
      #   geobike[i, frame_size_std := median_sizes[4]]
      # }
