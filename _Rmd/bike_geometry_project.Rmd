...
frame_size_predictor <- function(geobike){
  ...
  letter_order <- c("XXXXS", "XXXS", "XXS", "XS", "S", "SM", "M", "ML", "L", "LXL", "L", "XXL", "XXXL")
...
# tt + st + stack + reach + hta + sta + fc + cs + ht + ac = 0.906
  ...
  geobike[, front_chord := sqrt((head_x - bottom_x)^2 + (head_y - bottom_y)^2)]
  ...
  geobike[, frame_size_numeric := as.numeric(factor(frame_size, ordered = TRUE))]
  ...
  K <- length(x_cols)
  mono <- rep(0, K)
  mono[which(x_cols %in% c("frame_size_numeric"))] <- 1
...
#  geobike[model == "Sage Storm King 2025", rider_height_pred]
